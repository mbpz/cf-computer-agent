...
describe("GraphProjectionRepository D1 authorization contract", () => {
  beforeEach(async () => {
    ...
    await env.DB.prepare("INSERT INTO project_tasks (project_id, member_id, task_id, created_at) VALUES ('graph-project-a', 'graph-a', 'graph-task-a', ?), ('graph-project-b', 'graph-b', 'graph-task-b', ?)").bind(Date.parse(now), Date.parse(now)).run();
