...
describe("new-environment restore plan", () => {
  it("maps identities and preserves revision/citation/original operations without writes", async () => {
    ...
    const plan = await planKnowledgeRestore(pkg, { expectedSchemaFingerprint: "migrations-0021", memberMap: { "member-1": "target-admin" } });
  ...
  it("fails closed for schema, unmapped publishers, duplicate targets and broken citations", async () => {
    ...
    const unmapped = await planKnowledgeRestore(pkg, { expectedSchemaFingerprint: "migrations-0021", memberMap: {} });
    expect(unmapped.errors.map((error) => error.code)).toContain("IDENTITY_UNMAPPED");
    const duplicate = await planKnowledgeRestore(pkg, { expectedSchemaFingerprint: "migrations-0021", memberMap: { "member-1": "target-admin" }, targetMemberIds: ["target-admin"] });
