...
describe("frontend route contract", () => {
  ...
  it("keeps all ten formal page sources on numbered pagination only", async () => {
    ...
    for (const [path, source] of Object.entries(formalClientSources)) {
      expect(source, path).not.toMatch(/\bnextCursor\b|[?&]cursor=|\bonLoadMore\b|Load More|Load more|加载更多/u);
    }
