...
describe("frontend search data", () => {
  it("requests a numbered encoded query with filters and normalizes citations", async () => {
    const requester = vi.fn(async (input: string | URL | Request) => {
      expect(String(input)).toBe("/api/knowledge/search?q=title+%2F+body&page=2&pageSize=20&tagId=tag-a&tagMode=and");
    ...
    await expect(loadSearchPage({ query: "  title / body  ", page: 2, pageSize: 20, tagIds: ["tag-a"], tagMode: "and", requester })).resolves.toEqual({
