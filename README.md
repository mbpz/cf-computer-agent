...
- Admin review queue and detail reads now have same-query retry, explicit authorization/not-found states, response-ID validation, stale-navigation protection and in-app queue/detail navigation ([D02-R1 local evidence](./docs/product/2026-09-16-admin-review-read-recovery-evidence.md)). Read retries do not replay decisions; navigation invalidates pending publication previews. Frontend in-flight locks are not server idempotency. Publication/index/notification convergence and signed-browser acceptance remain open.
