...
function withPersistedAssistant(
  ...
  return new ReadableStream<Uint8Array>({
    start(controller) {
      ...
      if (workScope) void workScope.run(pump).catch(() => undefined);
      else void pump().catch(() => undefined);
    },
    async cancel(reason) {
      disconnected = true;
      try { workScope?.markUncertain("BACKGROUND_WORK_FAILED"); } catch { /* sealed scope already retains the producer result */ }
