...
import type { MembersRepositoryPort } from "./repository";
...
export class MembersService {
  ...
  private async insertWithConflictRecovery(identity: AccessIdentity, role: "admin" | "contributor"): Promise<Member> {
    ...
    if (role !== "admin" || !(await this.repository.hasActiveAdmin())) throw new AppError("MEMBER_CONFLICT", "Member login conflicted", 409, true);
  ...
  private async resolveExisting(member: Member): Promise<Member> {
    ...
    void this.repository.touchLastSeenIfStale(member.id, now.toISOString(), staleBefore).catch(() => {
