...
/**
 * Optional Workers AI adapter for text-like rich assets. PDFs, DOCX, and XLSX
 * use local bounded recovery paths; images use the separate vision adapter.
 */
export class WorkersAiMarkdownConverter implements AssetMarkdownConverter {
  ...
  async toMarkdown(input: { name: string; blob: Blob }): Promise<AssetMarkdownConversionResult> {
    ...
    if (contentType === "application/vnd.openxmlformats-officedocument.presentationml.presentation") {
      return { format: "markdown", data: (await recoverPptxMarkdown(await input.blob.arrayBuffer())).markdown };
    }
