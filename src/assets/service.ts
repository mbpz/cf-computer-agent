...
export class AssetService {
  ...
  async create(input: AssetUploadInput): Promise<AssetWithJob> {
    ...
    try {
      ...
      await observeStorage(this.options.workScope, () => this.requireStorage().delete(objectKey)).catch(error => { this.notifyFailure(error); });
  ...
  async cancel(ownerId: string, assetId: string): Promise<void> {
    ...
    await observeStorage(this.options.workScope, () => this.requireStorage().delete(removed.objectKey)).catch(error => { this.notifyFailure(error); });
  }
  ...
  private async processRecord(current: AssetWithJob): Promise<AssetWithJob> {
    ...
    try {
      ...
      await observeStorage(this.options.workScope, () => this.requireStorage().delete(parsedKey)).catch(error => { this.notifyFailure(error); });
  ...
  private async parseRichAsset(asset: AssetRecord, bytes: ArrayBuffer) {
    ...
    if (asset.contentType === "application/vnd.oasis.opendocument.text" && !converter) {
      return parseSource({ kind: "markdown", content: (await recoverOpenDocumentMarkdown(bytes, "odt")).markdown });
    }
    if (asset.contentType === "application/vnd.oasis.opendocument.spreadsheet" && !converter) {
      return parseSource({ kind: "markdown", content: (await recoverOpenDocumentMarkdown(bytes, "ods")).markdown });
    }
    if (asset.contentType === "application/vnd.apple.numbers" && !converter) {
      return parseSource({ kind: "markdown", content: (await recoverOpenDocumentMarkdown(bytes, "numbers")).markdown });
    }
    if (asset.contentType === "application/vnd.openxmlformats-officedocument.presentationml.presentation" && !converter) {
      return parseSource({ kind: "markdown", content: (await recoverPptxMarkdown(bytes)).markdown });
    }
    if (!converter) {
      throw new AppError("ASSET_PARSER_UNSUPPORTED", "Asset type is not supported by this parser", 422);
    }
