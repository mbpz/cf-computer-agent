...
export class GraphProjectionRepository implements GraphProjectionRepositoryPort {
  ...
  async listKnowledge(memberId: string, limit = 100): Promise<GraphKnowledgeRecord[]> {
    ...
    const chunks = revisionIds.length === 0 ? { results: [] as CitationRow[] } : await this.db.prepare(
      `SELECT revision_id, id AS chunk_id FROM chunks WHERE status = 'active' AND revision_id IN (${revisionIds.map(() => "?").join(",")}) ORDER BY revision_id ASC, ordinal ASC LIMIT ?`,
    ).bind(...revisionIds, 200).all<CitationRow>();
  ...
  async listTasks(memberId: string, limit = 100): Promise<GraphTaskRecord[]> {
    const rows = await this.db.prepare(
      `SELECT id, title, status, updated_at, priority, progress
       FROM tasks WHERE member_id = ? ORDER BY id ASC LIMIT ?`,
  ...
  async listProjects(memberId: string, limit = 100): Promise<GraphProjectRecord[]> {
    ...
    const rows = await this.db.prepare(
      `SELECT id, title, status, updated_at, progress
       FROM projects WHERE member_id = ? ORDER BY id ASC LIMIT ?`,
  ...
  async listGoals(memberId: string, limit = 100): Promise<GraphGoalRecord[]> {
    const rows = await this.db.prepare(
      `SELECT id, title, status, updated_at, progress
       FROM goals WHERE member_id = ? ORDER BY id ASC LIMIT ?`,
  ...
  async listInbox(memberId: string, limit = 100): Promise<GraphInboxRecord[]> {
    const rows = await this.db.prepare(
      `SELECT id, content, status, updated_at, promoted_task_id
       FROM inbox_items WHERE member_id = ? ORDER BY id ASC LIMIT ?`,
  ...
  async listCalendar(memberId: string, limit = 100): Promise<GraphCalendarRecord[]> {
    const rows = await this.db.prepare(
      `SELECT id, title, status, kind, updated_at, task_id, project_id
       FROM calendar_events WHERE member_id = ? ORDER BY id ASC LIMIT ?`,
  ...
  async listTimeline(memberId: string, limit = 100): Promise<GraphTimelineRecord[]> {
    ...
    const rows = await this.db.prepare(`SELECT id, project_id, kind, title, status, updated_at FROM project_timeline_items WHERE member_id = ? AND kind IN ('meeting', 'decision', 'action_item') ORDER BY id ASC LIMIT ?`).bind(memberId, limitValue(limit)).all<TimelineRow>();
    ...
    const [taskLinks, dependencies, projectGoals, projectTasks, calendar, inbox, timeline] = await Promise.all([
      ...
      this.db.prepare(
        `SELECT 'inbox' AS source_kind, id AS source_id, 'task' AS target_kind,
         ...
         FROM inbox_items WHERE member_id = ? AND promoted_task_id IS NOT NULL ORDER BY id ASC LIMIT ?`,
      ).bind(memberId, limitValue(limit)).all<RelationRow>(),
      this.db.prepare(
