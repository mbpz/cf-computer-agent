...
export function AssetAvailabilityPanel({ locale, memberId, title = "" }: { locale?: LocaleRuntime; memberId?: string; title?: string }) {
  ...
  const message = frontendText(locale, state.kind === "loading" ? "SUBMIT_ASSET_LOADING"
    ...
    : state.value.storageEnabled ? "ASSET_FLOW_MEMBER_REQUIRED" : "SUBMIT_ASSET_NOT_CONFIGURED");
  return <div data-asset-availability={state.kind} aria-busy={state.kind === "loading"}>
    {state.kind === "ready" && state.value.storageEnabled && memberId ? <AssetUploadPanel key={memberId} memberId={memberId} maxBytes={state.value.maxBytes} title={title} locale={locale} /> : <AssetDropzone locale={locale} disabledMessage={message} />}
