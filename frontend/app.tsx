...
function AdminMembersRoute({ locale }: { locale: LocaleRuntime }) {
  ...
  return <MembersPage locale={locale} loading={state.kind === "loading"} members={state.kind === "ready" ? state.members : []} nextCursor={state.kind === "ready" ? state.nextCursor : null} pendingIds={pendingIds} actionError={actionError} onLoadMore={loadMore} onStatusChange={changeStatus} />;
}
