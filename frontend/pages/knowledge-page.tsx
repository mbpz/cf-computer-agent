...
export function KnowledgePage({ state, onLoadMore, locale }: { state: KnowledgeState; onLoadMore?: () => void; locale?: LocaleRuntime }) {
  ...
  return <section className="space-y-6"><div><h1 className="text-2xl font-semibold">{frontendText(locale, "KNOWLEDGE_TITLE")}</h1><p className="mt-1 text-sm text-muted-foreground">{frontendText(locale, "KNOWLEDGE_DESCRIPTION")}</p></div>{state.items.length ? <div className="grid gap-4 md:grid-cols-2">{state.items.map((item) => <KnowledgeCard key={item.id} locale={locale} item={item} />)}</div> : <Card><CardContent className="p-6 text-sm text-muted-foreground">{frontendText(locale, "KNOWLEDGE_EMPTY")}</CardContent></Card>}{state.nextCursor && <Button variant="outline" onClick={onLoadMore}>{frontendText(locale, "KNOWLEDGE_LOAD_MORE")}</Button>}</section>;
}
