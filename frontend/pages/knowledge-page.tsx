...
export function KnowledgePage({ state, onLoadMore, locale, recent = [], recentResearch = [] }: { state: KnowledgeState; onLoadMore?: () => void; locale?: LocaleRuntime; recent?: readonly RecentKnowledgeItem[]; recentResearch?: readonly RecentResearchItem[] }) {
  ...
  return <section className="space-y-6"><div><h1 className="text-2xl font-semibold">{frontendText(locale, "KNOWLEDGE_TITLE")}</h1><p className="mt-1 text-sm text-muted-foreground">{frontendText(locale, "KNOWLEDGE_DESCRIPTION")}</p></div>{recent.length > 0 && <RecentKnowledgePanel locale={locale} items={recent} />}{recentResearch.length > 0 && <RecentResearchPanel locale={locale} items={recentResearch} />}{state.items.length ? <div className="grid gap-4 md:grid-cols-2">{state.items.map((item) => <KnowledgeCard key={item.id} locale={locale} item={item} />)}</div> : <PageState kind="empty" title={frontendText(locale, "KNOWLEDGE_EMPTY")} />}{state.nextCursor && <Button variant="outline" disabled={state.pending} onClick={onLoadMore}>{frontendText(locale, "KNOWLEDGE_LOAD_MORE")}</Button>}</section>;
}
