...
export function TasksPage({ state, filters, locale, pending = false, localError, actionPendingId, onRetry, onFilterChange, onPageChange, onPageSizeChange, onStatusChange, onDelete }: {
  ...
  return <section className="space-y-5">
    ...
    {state.items.length ? <div className="space-y-3">{state.items.map((task) => <Card key={task.id}><CardContent className="flex flex-col gap-3 p-4 sm:flex-row sm:items-center sm:justify-between"><div className="min-w-0"><div className="flex flex-wrap items-center gap-2"><p className="font-medium">{task.title}</p><Badge variant="outline">{frontendText(locale, taskStatusKey(task.status))}</Badge><Badge variant="outline">{frontendText(locale, taskPriorityKey(task.priority))}</Badge></div>{task.notes && <p className="mt-1 truncate text-sm text-muted-foreground">{task.notes}</p>}</div><div className="flex flex-wrap gap-2"><Button size="sm" variant="outline" disabled={actionPendingId === task.id} onClick={() => onStatusChange?.(task.id, task.status === "done" ? "todo" : "done")}>{frontendText(locale, task.status === "done" ? "TASKS_REOPEN" : "TASKS_COMPLETE")}</Button><Button size="sm" variant="destructive" disabled={actionPendingId === task.id} onClick={() => onDelete?.(task.id)}>{frontendText(locale, "TASKS_DELETE")}</Button></div></CardContent></Card>)}</div> : <PageState kind="empty" title={frontendText(locale, "TASKS_EMPTY")} />}
