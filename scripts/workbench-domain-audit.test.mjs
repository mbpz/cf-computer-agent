...
test("D02-R1 preserves D01-B2, D01-B1, D01-A, M02 and R0 snapshots without backdating recovery evidence", () => {
  ...
  const b2 = readFileSync(resolve(repositoryRoot, "docs/operations/evidence/2026-09-15-workbench-d01b2-domain-audit.md"), "utf8");
  assert.match(b2, /Missing preview is an error rather than empty and initial error has no retry/u);
  assert.doesNotMatch(current, /Missing preview is an error rather than empty and initial error has no retry/u);
  assert.match(current, /same-object retry, explicit 404\/401\/403 states/u);
