...
test("pins the reviewed bytes of all forward migrations", async () => {
  ...
  assert.equal(result.code, 0, result.output);
    assert.match(result.output, /^\[pass\] migration-files count=18$/mu);
});
...
test("accepts only the exact reviewed Wrangler ledger prefixes", async () => {
  ...
  await withLedger(ledger(names), async (path) => {
    ...
    assert.match(result.output, /0010_m2_chunk_metadata\.sql,0011_m4_saved_views\.sql,0012_m5_private_notes\.sql,0013_m6_research_reports\.sql,0014_m6_research_run_plan\.sql,0015_m6_research_plan_steps\.sql,0016_m6_research_subquestions\.sql,0017_m6_research_queries\.sql,0018_m6_research_quota\.sql$/mu);
    assert.match(result.output, /0013_m6_research_reports.sql,0014_m6_research_run_plan.sql,0015_m6_research_plan_steps.sql,0016_m6_research_subquestions.sql,0017_m6_research_queries.sql,0018_m6_research_quota.sql$/mu);
  });
