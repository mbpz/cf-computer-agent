...
- [x] 提供“开始专注 / 暂停 / 完成 / 放弃”状态流。
- [x] 专注状态必须与 task/member 绑定，重复点击幂等。
- [x] 当前专注项只返回本人数据；不引入实时广播或付费队列。
- [x] 页面刷新后可恢复当前状态，结束时写入完成时间并同步 Calendar focus block。
...
C1–C6 本地证据已完成，P3-A capture 分类与 P4-A 项目时间线的本地纵向切片也已完成。若后续测试暴露任务、日程、聚合、专注、复盘、capture 或项目时间线的 owner、时间范围、状态机、确定性 key 或数量边界不足，先修复数据边界，不通过增加前端隐藏逻辑绕过。
