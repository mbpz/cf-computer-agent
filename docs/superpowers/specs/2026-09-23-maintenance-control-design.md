...
状态：2026-09-23 用户确认本文书面规格——独立运维控制入口、专用凭证、协调器二次校验、仅本地验证、不加后台按钮。[实施计划](../plans/2026-09-23-maintenance-control.md)已编排；**R05.1 已提交 `a329ce9`（34 项鉴权测试通过）；R05.2 完整回归 13 文件 / 291 测试通过、类型检查通过，本地提交因审批超时未执行，R05 父项未完成**。
...
- [x] 2026-09-23 用户确认本文的详细协议与边界。
- [x] 审阅通过后使用 writing-plans 形成[实施计划](../plans/2026-09-23-maintenance-control.md)，执行步骤按实际证据逐项勾选，不以计划代替实现。
- [ ] 顺序实现、回归和留证后关闭 R05。

这些是 R05 内部设计/交付步骤，不改变恢复父项统计：**4/25 完成，21 项未完成**。产品清单仍为 30 个未关闭父项，与恢复主线重叠不相加。R05.2 的 GREEN 已由用户贴回结果确认；下一动作是完成本地提交，再进入 R05.3，不执行生产变更或 R06。
