...
状态：2026-09-23 用户确认本文书面规格——独立运维控制入口、专用凭证、协调器二次校验、仅本地验证、不加后台按钮。[实施计划](../plans/2026-09-23-maintenance-control.md)已编排；**R05.1–R05.5 尚未执行，无 R05 实现/测试完成声明**。
...
- [x] 审阅通过后使用 writing-plans 形成[实施计划](../plans/2026-09-23-maintenance-control.md)，执行步骤尚未勾选。
- [ ] 顺序实现、回归和留证后关闭 R05。

这些是 R05 内部设计/交付步骤，不改变恢复父项统计：**4/25 完成，21 项未完成**。产品清单仍为 30 个未关闭父项，与恢复主线重叠不相加。下一动作是按实施计划执行 R05.1 运行时探针与失败测试；本次仅提交计划，不声称实现完成，不执行生产变更或 R06。
