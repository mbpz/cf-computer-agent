...
rtk npm run build
Vite: built in 215ms
landing/build contract: 96 passed, 0 failed
...
- `shared/workspace-route-capabilities.ts` registers `{ id: "graph", path: "/graph", pageKind: "graph", availability: "ready" }`; `frontend/app.tsx` imports `GraphRoute` and dispatches it only from the `graph` page kind.
- `frontend/pages/graph-page.tsx` imports `GraphCanvas`; the source contract scans all frontend TypeScript sources and requires `frontend/components/graph/graph-canvas.tsx` to be the only source containing Cytoscape references. The canvas has no eager `from "cytoscape"` import and uses exactly `import("cytoscape")`.
