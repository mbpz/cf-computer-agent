...
- [ ] `PAR-013` P1/M2 ODT/ODS/Numbers；状态：L/W/D；ODT/ODS 已本地有界读取 `content.xml`，恢复标题/段落/列表、Sheet/表格/A1 范围；Numbers 的 IWA 二进制格式在免费层无安全解析器，已明确 `ASSET_NUMBERS_PARSE_UNSUPPORTED` 降级并禁止送入 AI，待后续专用 parser fixture 后再勾选。证据：`src/assets/odf.ts`、`src/assets/ai-markdown.ts`、`src/assets/service.ts`、`test/unit/odf.test.ts`；命令：`rtk npx vitest run test/unit/odf.test.ts test/unit/ai-markdown.test.ts test/unit/assets-service.test.ts`。
