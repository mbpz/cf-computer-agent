...
- [ ] `PAR-020` P1/M2 重新解析；验收：新 parser version，不覆盖已发布 Revision。候选构建器、D1 queued/processing/indexed 状态机、幂等持久化和管理员 API 已完成：`src/sources/reparse.ts`、`src/sources/reparse-service.ts`、`src/sources/reparse-repository.ts`、`migrations/0006_m2_source_reparse.sql`，路由为 `POST /api/admin/source-versions/:id/reparse` 与 `GET /api/admin/reparse-jobs/:id`；`test/unit/source-reparse.test.ts`、`test/unit/source-reparse-service.test.ts`、`test/worker/m2-reparse.test.ts` 已验证旧 SourceVersion/Revision 不变。仍待管理员确认后将候选物化为新 SourceVersion/Revision。
