...
- [x] `RES-012` P1/M6 研究取消；状态：L/W；验收：owner 可取消 draft/running/paused ResearchRun；取消后报告/查询写入 fail-closed，不再调用 AI，既有查询、子问题、计划和报告版本保持可审计。证据：`src/ai/research-report-service.ts`、`src/research/repository.ts`、`src/routes/library.ts`、`test/worker/m1-api.test.ts`；接口：`POST /api/knowledge/:knowledgeItemId/research-runs/:runId/cancel`；命令：`rtk npm run typecheck && rtk npx vitest run test/worker/m1-api.test.ts -t 'research|Research'`。
