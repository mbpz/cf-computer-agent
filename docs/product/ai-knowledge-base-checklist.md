...
- [ ] `PAR-020` P1/M2 重新解析；验收：新 parser version，不覆盖已发布 Revision。候选构建器、D1 queued/processing/indexed 状态机、幂等持久化、管理员重解析与确认物化已完成：`src/sources/reparse.ts`、`src/sources/reparse-service.ts`、`src/sources/reparse-repository.ts`、`migrations/0006_m2_source_reparse.sql`，路由为 `POST /api/admin/source-versions/:id/reparse`、`GET /api/admin/reparse-jobs/:id` 与 `POST /api/admin/reparse-jobs/:id/promote`；确认会创建新的 review_pending Submission/Source/SourceVersion，旧 SourceVersion/Revision 不变。仍待将物化 Submission 接入现有管理员发布审核并生成新 Revision。证据：`test/unit/source-reparse.test.ts`、`test/unit/source-reparse-service.test.ts`、`test/worker/m2-reparse.test.ts`。
