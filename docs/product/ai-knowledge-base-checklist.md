...
- [ ] `PAR-020` P1/M2 重新解析；验收：新 parser version，不覆盖已发布 Revision。候选构建器已完成：`src/sources/reparse.ts` 生成 `m2-v1`/`m2-v1`、递增 ordinal 和稳定 fingerprint，并通过 `test/unit/source-reparse.test.ts` 3/3；D1 queued/processing 状态机、幂等持久化和管理员确认发布仍待实现。
