...
- Migrations `0001` through `0009_m2_chunk_status.sql` are forward-only. Once applied remotely, do not edit, reverse, or delete them. Never delete D1 rows/tables or Durable Object/VFS state to roll back a Worker.
...
- the actual Wrangler `d1_migrations` ledger is an exact prefix of the reviewed ordered set `0001` through `0009`, with no missing, renamed, reordered, or extra row;
- the local reviewed set contains the complete ordered `0001` through `0009` forward-migration set;
...
The verifier requires the reviewed pre-`0004` state, or an exact post-`0004` through post-`0009` prefix, and fails closed for any unexpected ledger state. Do not edit or replay SQL directly; investigate and stop. Keep the restricted ledger file out of the repository and never attach raw command output.
