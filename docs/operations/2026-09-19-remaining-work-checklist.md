...
- [ ] **R11 — 补发布/schema 防漂移门禁。** 本地已完成 51 条 migration 文件 hash 校验、R11 维护发布合同 34/34、配置 smoke 8/8，以及缺迁移/错库/旧 SQL hash/意外新增 migration 的失败闭合断言；生产侧批准候选 hash、实际迁移前缀、待迁移清单、目标 binding 和同版本验收仍待通过 Dashboard 只读核对。见[R11 schema 防漂移预检证据](evidence/2026-09-24-r11-schema-drift-preflight.md)。实际平台配置变更须批准，不自动每次 push 执行迁移。
