...
- [ ] **R11 — 补发布/schema 防漂移门禁。** 本地已完成 51 条 migration 文件 hash 校验、R11 维护发布合同 34/34、配置 smoke 8/8，以及缺迁移/错库/旧 SQL hash/意外新增 migration 的失败闭合断言；Dashboard 只读核对确认生产已应用 0001–0050，唯一 pending 为本地候选 `0051_calendar_reference_detach.sql`（SHA-256 `30b4c77fc4e3198a1c6937adc0d95ba0c8bf142e707d670f0b12a233db47058d`），D1 database id 与 binding 目标一致；当前 Worker 候选版本与生产 binding 的同版本验收、0051 的独立迁移批准和新鲜备份前置仍未完成。见[R11 schema 防漂移预检证据](evidence/2026-09-24-r11-schema-drift-preflight.md)。实际平台配置变更须批准，不自动每次 push 执行迁移。
