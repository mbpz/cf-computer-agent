...
用户最新要求“不需要 功能任务 checklist都完成就可以了”，不接受上一轮维护密钥激活提议。本轮停止推进生产运维主线，转回[功能补齐清单](../product/2026-09-12-personal-workbench-completion-audit.md#当前执行范围2026-09-25-用户调整)。以下严格 R01→R25 顺序仅适用于将来重新获准的运维工作，**不再阻塞当前功能实现与本地验证**。本次不配置维护密钥、不停写、不迁移、不备份、不发布；未执行的运维步骤不标成完成或自动豁免。

运维历史计数保留：25 项中 12 关闭（11 完成、R12 用户豁免），13 未关闭，最后推进点 R13。它不是本轮功能进度计数。后续任何生产变更仍须具体授权。
...
- [ ] **R13 — 批准并形成可追溯维护发布候选（当前）。** 分别明确提交、合并、推送及维护代码部署的允许范围；获准后执行相应步骤，记录精确候选、所需 binding/配置和回滚/退出策略。未经授权不操作远程仓库或发布。 本地候选及本次生产只读核对见[R13 记录](evidence/2026-09-24-r13-candidate-preflight.md)：已确认 `cc71530 → #9e25da19 → da1a0382 → 100%`，无需重复发布现有代码；待确认采用此基线及维护 Secret 激活的具体范围。备份豁免不代替生产变更批准。
- [ ] **R14 — 授权部署维护能力并核实写者覆盖。** 2026-09-24 最新只读核对确认当前 `cc71530` 已由 Cloudflare CI 成功部署，当前 100% 版本为 `da1a0382`（Build `#9e25da19`，见[R13 最新证据](evidence/2026-09-24-r13-candidate-preflight.md#5-2026-09-24-续办生产发布链已刷新)），`MAINTENANCE` Durable Object binding 和管理员维护控制 API 已出现；但生产 secrets 仍未配置 `MAINTENANCE_CONTROL_TOKEN`，因此 guarded fetch/Cron 未启用，不能验证生产 `status/capacity/beginDrain/resume`。见[guarded 部署证据](evidence/2026-09-22-r14-guarded-deployment.md)和[R14 生产激活预检](evidence/2026-09-22-r14-production-activation-preflight.md)。仍需确认所有可写入口使用该控制机制、旧版本/其他写者按方案停止或隔离，并验证控制鉴权。仅部署代码并不等于已经冻结数据；验证过程仍在批准范围内。
