...
- [ ] **R10 — 完成远程合成只读传输验证。** 2026-09-24 已通过 Cloudflare Dashboard D1 控制台完成 `SELECT 1` 与 `PRAGMA table_list` 只读探针，本地正式工具 `rtk npm run test:ops:d1-backup` 在 listener 权限下 21/21 通过；正式 REST transport 的真实窄权限只读 token、response envelope 和编码验证仍缺。见[R10 远程只读探针证据](evidence/2026-09-24-r10-remote-read-probe.md)。离线 fixture 不能替代真实接口验证；不访问生产私有内容。
