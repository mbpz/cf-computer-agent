...
- [ ] **R05 — 完成维护控制面的授权与重放防护。** 独立确认控制方案；本地证明未经授权不能关闸/开闸，窗口和 epoch 严格匹配、重复命令结果一致，旧命令不能重新开闸。业务 cookie/普通成员权限不自动具备维护控制权限。2026-09-23 用户确认[书面规格](../superpowers/specs/2026-09-23-maintenance-control-design.md)，[实施计划](../superpowers/plans/2026-09-23-maintenance-control.md)已编排；R05.1 已提交 `a329ce9`（34 项鉴权测试通过），R05.2 完整回归 13 文件 / 291 测试通过，待本地提交，R05.3–R05.5 尚未执行，不关闭父项。仅本地、不加后台按钮、不改生产配置。
