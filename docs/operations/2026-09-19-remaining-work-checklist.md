...
当前优先恢复主线共 **25 个检查点，3 个关闭，22 个剩余；下一项 R04**。R01/R02/R03 于 2026-09-20 关闭，见[真实入口本地证据](evidence/2026-09-20-maintenance-entry.md)和[D1 生命周期证据](evidence/2026-09-20-maintenance-d1-lifecycle.md)。前序 0051、备份可行性及备份工具不重复计入；旧测试记录见[迁移证据](../product/2026-09-17-calendar-reference-detach-evidence.md)、[备份工具证据](evidence/2026-09-18-d1-backup-tooling.md)、[协调器证据](evidence/2026-09-19-maintenance-coordinator.md)。本轮维护专项 55/55 及完整应用回归通过，未重跑备份工具测试。
...
- [x] **R02 — 接入真实 fetch/Cron 的本地测试路径。** 2026-09-20 完成本地共用入口及独立合成资源 harness。关闭时 HTTP 503、Cron 不进入业务；45 项专项和完整应用回归通过，见[候选及验证证据](evidence/2026-09-20-maintenance-entry.md)。index 明确 legacy，未新增生产 binding/启用生产维护；R04 仍未开始。
- [x] **R03 — 接管后台与隐式写生命周期。** 2026-09-20 完成本地 D1 facade、scope sealing/不确定状态、成员 last_seen/session/nonces 原始 Promise 观察及真实 guarded entry 回归；业务响应语义保留，失败或结果不明不会安全释放。见[D1 生命周期证据](evidence/2026-09-20-maintenance-d1-lifecycle.md)。生产入口仍为 legacy，R04 未开始。
...
- 2026-09-20 R03 收尾校验：维护专项 55/55、应用/专项类型检查、完整 `npm test`、构建、交付合同 28/28 和 `git diff --check` 通过；候选提交 `95bd921`。生产 Wrangler/生成 Env 未变，仍未执行生产变更。
- 后续游标：R04，尚未开始。本轮停在 R03 完成点，不执行生产变更。
