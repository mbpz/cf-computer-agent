...
- [ ] **R05 — 完成维护控制面的授权与重放防护。** 独立确认控制方案；本地证明未经授权不能关闸/开闸，窗口和 epoch 严格匹配、重复命令结果一致，旧命令不能重新开闸。业务 cookie/普通成员权限不自动具备维护控制权限。2026-09-23 用户确认独立运维入口、专用凭证及协调器二次校验，见[书面规格](../superpowers/specs/2026-09-23-maintenance-control-design.md)；当前为规格待审阅，尚无实施计划或实现，不关闭父项。仅本地、不加后台按钮、不改生产配置。
...
- 2026-09-23 当前游标：R05 架构方向已确认，书面规格待审阅，尚无实施计划或实现。R04 第五子项完成并关闭父项，新增 10 项跨存储组合及三处临时回退验证，维护专项 247 项、应用 smoke/unit/worker 分项和两套类型检查通过，见[收口证据](evidence/2026-09-23-maintenance-r04-completion.md)。这些是 R04 历史验证，不代表 R05 已通过。恢复主线 **4 关闭 / 21 剩余**；本轮不推送、不合并 main、不部署，不提升产品交付维度。
