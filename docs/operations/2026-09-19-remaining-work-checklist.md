...
- [ ] **R03 — 接管后台与隐式写生命周期。** 覆盖成员 last_seen、session/nonces 清理、嵌套 waitUntil；在业务 catch 之前观察原始失败，保持现有响应语义，失败或结果不明不能被当作安全完成。
...
- 后续游标：R03，尚未开始。本轮停在 R02 完成点，不执行生产变更。
