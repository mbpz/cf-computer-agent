...
- [x] **R03 — 接管后台与隐式写生命周期。** 2026-09-20 完成本地 D1 facade、scope sealing/不确定状态、成员 last_seen/session/nonces 原始 Promise 观察及真实 guarded entry 回归；业务响应语义保留，失败或结果不明不会安全释放。见[D1 生命周期证据](evidence/2026-09-20-maintenance-d1-lifecycle.md)。生产入口仍为 legacy；R04 已进入本地最小切片但尚未关闭。
...
- 后续游标：R04，已完成流/存储最小切片但尚未关闭。本轮不执行生产变更。
