...
Dashboard 的 Production secrets 列表仍只有既有密钥，未出现 `MAINTENANCE_CONTROL_TOKEN`。因此：

- guarded fetch/Cron 的准入路径已部署；
- `MaintenanceCoordinator` 的 `beginDrain`/`resume` 在生产会因缺少控制能力而返回 `CONTROL_UNAUTHORIZED`；
- 生产尚未具备可执行停写/恢复的完整条件；
