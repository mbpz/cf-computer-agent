...
| 生产分支 | `main` |
| 最近成功构建 | Build `#738458bc` |
| 构建 commit | `96ea03d6d85b1b5a1a3e9f0e0e1b4f4c0f2b3f6a` |
| 构建提交 | `docs: record production settings attestation` |
| 构建命令 | `npm run build` |
| 部署命令 | `npx wrangler deploy` |
| 根目录 | `/` |
| 当前 100% 生产版本 | `1f613a4f`（Dashboard 展示短版本标识；前序版本 `ff4e0667`、`686168b3`、`34949fd7`） |
| 当前生产 Worker | `memory-garden-agent` |

Dashboard 的构建历史显示 Build `#738458bc` 成功构建 `96ea03d`，部署页随后显示当前生产版本 `1f613a4f` 为 100% 流量。当前最新 source → build → production version 已可追溯；历史版本 `ff4e0667`、`686168b3`、`34949fd7` 保留为旧版本责任记录。部署历史的来源字段是 Wrangler，不能额外推断为 GitHub Actions。
...
当前 `main` 已包含 `850db38` 安全闸门、`2feb021` 管理员维护控制 API 和 R09 账本刷新；Dashboard 已确认 `96ea03d` / Build `#738458bc` / production `1f613a4f` 100%。最新 source-to-version 补证完成；代码内写者、DO、Cron、Automation、手工 Wrangler 与旧版本责任拆分见[2026-09-22 R09 writer ledger](2026-09-22-r09-writer-ledger.md)。
