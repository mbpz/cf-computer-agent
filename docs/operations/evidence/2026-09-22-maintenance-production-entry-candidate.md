...
- `src/index.ts` 导出 `MaintenanceCoordinator`，生产入口改为 `createWorkerEntry({ mode: "guarded", maintenance: env => env.MAINTENANCE.getByName("production") })`。
...
1. 为生产 Worker 配置 `MAINTENANCE_CONTROL_TOKEN` secret（只记录名称，不读取值）。
