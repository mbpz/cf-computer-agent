...
| --- | --- | --- |
| Cloudflare Dashboard Git 集成 | 2026-09-23 已确认 Build `#dee044b9`、commit `2be3c57303e1635a535f51506f3cc6479dbe6d23`；部署历史随后出现 `43e689ac`、`ca60bbc3` 两个 Wrangler 100% 版本 | source → build 已完成；当前页面未直接给出 build → 版本一一映射，仍需版本详情/部署日志及责任人、停写顺序和恢复顺序；不额外推断为 GitHub Actions |
...
- `850db38` 的安全闸门、`2feb021` 的维护控制 API 和 R09 账本刷新已进入构建链；当前 binding、Cron、Queue/Email 证据已刷新，但生产 100% 版本已从 `6025488e` 漂移到 `ca60bbc3`，其与最新构建的直接映射仍待补证。
