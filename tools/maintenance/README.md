...
configuration or modify production bindings. The standalone local operations
HTTP adapter remains a later recovery R05 step. The merged application retains
main’s existing administrator API, capacity snapshot and production entry switch;
local tests do not prove that production maintenance is enabled.
