...
function command(machine, text, timeoutMs = 8000) {
  ...
  return new Promise((resolve, reject) => {
    ...
    const timer = setTimeout(() => finish(new Error('Recovery guest command timed out')), timeoutMs);
...
export async function runRecoveryProbe({ profile, authorize, inspect, waitForFreshGrant, onProgress = () => {} }) {
  ...
  try {
    ...
    await session.ready;
    requireEvidence((await command(machine, 'ifconfig eth0 192.168.86.100 netmask 255.255.255.0 up && ip route replace default via 192.168.86.1 dev eth0')).exitCode === 0, 'guest route');
    const once = await command(machine, `wget -T 3 -qO /tmp/once-result --post-data=once ${url}/once && cat /tmp/once-result`);
    requireEvidence(once.exitCode === 0 && once.output.trim() === 'ONCE-ACK', 'initial POST');
    requireEvidence((await command(machine, `(wget -T 8 -qO /tmp/live-stream ${url}/hold >/tmp/hold-log 2>&1; echo $? > /tmp/hold-status) &`)).exitCode === 0, 'held request started');
    requireEvidence((await command(machine, 'for n in 1 2 3 4 5; do test -s /tmp/live-stream && break; sleep 1; done; cat /tmp/live-stream')).output.trim() === 'LIVE-STREAM', 'guest received held response');
    onProgress('saving');
    ...
    requireEvidence((await command(machine, 'cat /tmp/once-result')).output.trim() === 'ONCE-ACK', 'restored guest file');
    requireEvidence((await command(machine, `wget -T 1 -qO /tmp/offline ${url}/offline`, 5000)).exitCode !== 0, 'restore must be offline');
    ...
    const fresh = await command(machine, `wget -T 3 -qO /tmp/after ${url}/after && cat /tmp/after`);
